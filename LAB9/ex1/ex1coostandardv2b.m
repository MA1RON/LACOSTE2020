...
%% calore disperso
% north
qnorth = (TTmat(1,ny2-1) - TTmat(1,ny2))*cond*dx/(dy/2);
jy = ny2;
for jx = 2:nx2-1
    qnorth = qnorth + (TTmat(jx,jy-1) - TTmat(jx,jy))*cond*dx/dy;
end
qnorth = qnorth + (TTmat(nx2,ny2-1) - TTmat(nx2,ny2))*cond*dx/(dy/2);
fprintf('Il calore disperso a nord vale: %.3f W.\n', qnorth)

% west
qwest = (TTmat(2,1) - TTmat(1,1))*cond*dy/(dx/2);
jx = 1;
for jy = 2:ny2-1
    qwest = qwest + (TTmat(jx+1,jy) - TTmat(jx,jy))*cond*dy/dx;
end
qwest = qwest + (TTmat(jx+1,ny2) - TTmat(jx,ny2))*cond*dy/(dx/2);
fprintf('Il calore disperso a ovest vale: %.3f W.\n', qwest)

fprintf('Calore totale disperso: %.3f W.\n',qnorth+qwest)
