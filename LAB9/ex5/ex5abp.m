...
dxvec=[1e-2, 5e-3, 2e-3, 1e-3]; % Per studio di grid independence (seconda richiesta)

%% Iterazioni con diversi dx, dy e corrispondente numero di nodi
for i=1:length(dxvec)
    dx=dxvec(i);
    dy=dx;
    ...
    %% Controllo delle condizioni al contorno
    if i==1 %Controllo solo alla prima iterazione
        rr1=reshape(rhs(1:n1*(n4-1)),n1,n4-1);
        rr2=reshape(rhs(n1*(n4-1)+1:end),n2,n3-n4+1);
        rrr=[[rr1;NaN*ones(n2-n1,n3-n4)],rr2];
        figure(1)
        surf(xmat,ymat,rrr')
        grid minor
    ...
    %% Risolvo il problema e plotto la soluzione
    T=A\rhs;
    T1=reshape(T(1:n1*(n4-1)),n1,n4-1);
    T2=reshape(T(n1*(n4-1)+1:end),n2,n3-n4+1);
    T_reshape=[[T1;NaN*ones(n2-n1,n4-1)],T2];
    if i==1 %Plotto solo la soluzione corrispondente alla prima iterazione
        figure(2)
    ...
    % In T_save salvo i valori di temperatura corrispondenti ai nodi della
    % prima soluzione, per le diverse soluzioni che ottengo; utilizzerò poi
    % T_save per lo studio di grid independence.
    r=round((nvec(i)-1)/(nvec(1)-1));
    T_save(:,i)=[reshape(T1(1:r:end,1:r:end),[],1);reshape(T2(1:r:end,1:r:end),[],1)];
    ...
    % Il valore q_2 [W] corrisponde alla dissipazione di calore considerando
    ...
    L=1e-2;
    q_2=q_1*L*(L/(2*a)); % Moltiplico il calore per unità di lunghezza per la lunghezza del chip in direzione normale alla figura e poi "proporziono" rispetto alla direzione orizzontale.
end

%% Studio di grid independence
% Calcolo e plotto l'errore rispetto alla soluzione numerica più accurata
for i=1:length(dxvec)-1
    err(i)=norm(T_save(:,i)-T_save(:,end))/norm(T_save(:,end));
end
figure(3)
loglog(dxvec(1:end-1),err,'-o','Linewidth',2)
grid minor
xlabel('dx [m]')
