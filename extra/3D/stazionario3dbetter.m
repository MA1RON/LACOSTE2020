...
% con surf + hold on
op = .1;
for jz = 1:nn % a strati di z
    surf(xmat(:,:,jz),ymat(:,:,jz),zmat(:,:,jz),TTmat(:,:,jz),'FaceAlpha',op,'EdgeColor','none')
    hold on
end
for jy = 1:nn % a strati di y
    surf(reshape(xmat(:,jy,:),nn,nn),reshape(ymat(:,jy,:),nn,nn),reshape(zmat(:,jy,:),nn,nn),reshape(TTmat(:,jy,:),nn,nn),'FaceAlpha',op,'EdgeColor','none')
    hold on
end
for jx = 1:nn % a strati di x
    surf(reshape(xmat(jx,:,:),nn,nn),reshape(ymat(jx,:,:),nn,nn),reshape(zmat(jx,:,:),nn,nn),reshape(TTmat(jx,:,:),nn,nn),'FaceAlpha',op,'EdgeColor','none')
    hold on
end
